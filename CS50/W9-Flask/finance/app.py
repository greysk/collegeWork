...
def create_transactions():
    ...
    db.execute("CREATE UNIQUE INDEX IF NOT EXISTS uid ON transactions(uid);")
    db.execute("CREATE INDEX IF NOT EXISTS user_id ON transactions(user_id);")
    db.execute("CREATE INDEX IF NOT EXISTS share ON transactions(share);")
...
def index():
    ...
    user_data = {
        ...
        'stocks': db.execute("""SELECT share, sum(share_count) as count
                                FROM transactions WHERE user_id=?
                                    AND share NOT LIKE 'Cash Deposit'
                                GROUP BY share;""", session.get("user_id")),
        'total_holdings': 0
        }
    for stock in user_data['stocks']:
        stock_data = lookup(stock['share'])
...
def buy():
    ...
    if request.method == "POST":
        ...
        users_cash = db.execute("SELECT cash FROM users WHERE id=?",
                          session.get("user_id"))[0]['cash']
        print(users_cash)
        if total_cost > users_cash:
            return apology(f"Cost ({total_cost}) greater than cash"
                           f" available ({users_cash})")
        # Add purchase to transactions database
        db.execute(f"""
            INSERT INTO transactions(
                user_id, share, share_count, share_price, dt)
            VALUES(?, ?, ?, ?, ?);""", session.get("user_id"), symbol, shares, stock_price, datetime.datetime.today())
        # Updated user's available cash
        db.execute("UPDATE users SET cash=? WHERE id=?",
                   (users_cash - total_cost), session.get('user_id'))
...
def history():
    ...
    # [ ] Implement
    transactions = db.execute("""SELECT share, share_count, share_price, dt
                                    FROM transactions WHERE user_id=?
                                 ORDER BY dt DESC;""", session.get("user_id"))
    print(transactions)
...
def register():
    ...
    if request.method == "POST":
        ...
        db.execute("""INSERT INTO transactions (
                        user_id, share, share_count, share_price, dt)
                    VALUES(?, ?, ?, ?, ?);""", user_id, 'Cash Deposit', 1,
                    10000.00, datetime.datetime.now())
...
def sell():
    ...
    if request.method == "POST":
        ...
        shares = int(shares)
        # Obtain owned stocks
        stocks = {stock['share']: int(stock['count']) for stock in db.
                        execute("""SELECT share, sum(share_count) as count
                                   FROM transactions
                                     WHERE user_id=?
                                        AND share NOT LIKE 'Cash Deposit'
        ...
        db.execute("UPDATE users SET cash=? WHERE id=?;",
                   (db.execute("SELECT cash FROM users WHERE id=?;",
                    ...
                    + money_gained),
                   user_id)
        return render_template("sell.html", success=True, stocks=[{'share': key} for key in stocks.keys()])
    else:
        stocks = db.execute("""SELECT share FROM transactions WHERE user_id=?
                                    AND share NOT LIKE 'Cash Deposit'
                               GROUP BY share;""", session.get("user_id"))
